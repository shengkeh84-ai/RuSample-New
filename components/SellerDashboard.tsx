...
const SellerDashboard: React.FC<SellerDashboardProps> = ({ onLogout }) => {
  ...
  const [overviewTab, setOverviewTab] = useState<'REVIEWS' | 'PRODUCTS' | 'ORDERS' | 'ANALYTICS'>('REVIEWS');
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (mediaItems.length + newFiles.length > 9) { alert("Maximum 9 files allowed."); return; }
      const newMediaItems: MediaItem[] = newFiles.map(file => ({ id: `new-${Date.now()}-${Math.random()}`, url: URL.createObjectURL(file), file: file, type: 'NEW', isVideo: file.type.startsWith('video/') }));
  ...
  const handleDragOver = (e: React.DragEvent, index: number) => { e.preventDefault(); if (draggedItemIndex === null || draggedItemIndex === index) return; const newItems = [...mediaItems]; const draggedItem = newItems[draggedItemIndex]; newItems.splice(draggedItemIndex, 1); newItems.splice(index, 0, draggedItem); setDraggedItemIndex(index); setMediaItems(newItems); };
  const handleDragEnd = () => { setDraggedItemIndex(null); };
  const handleUploadClick = () => { if (fileInputRef.current) fileInputRef.current.click(); };

  const handleEditClick = (product: any) => {
      setEditingId(product.id); setProductTitle(product.title); setProductCategory(product.category); setProductDesc(product.description || ''); setQuantity(product.stock); setOzonLink(product.platform_links?.ozon || ''); setWbLink(product.platform_links?.wb || ''); setReqPhoto(product.requirements?.photo || true); setReqVideo(product.requirements?.video || false);
      const existing: MediaItem[] = (product.images || []).map((url: string, idx: number) => ({ id: `exist-${idx}-${Date.now()}`, url: url, type: 'EXISTING', isVideo: isVideoUrl(url) }));
      setMediaItems(existing); setCurrentView('CREATE_PRODUCT');
  };

  const resetForm = () => { setEditingId(null); setProductTitle(''); setProductDesc(''); setQuantity(10); setOzonLink(''); setWbLink(''); setMediaItems([]); setReqPhoto(true); setReqVideo(false); };

  const handlePublish = async (e: React.FormEvent) => {
    e.preventDefault(); if (!realUser) return; setIsPublishing(true);
    try {
        const finalImageUrls: string[] = [];
        for (const item of mediaItems) {
            if (item.type === 'EXISTING') { finalImageUrls.push(item.url); } 
            else if (item.type === 'NEW' && item.file) {
                const fileExt = item.file.name.split('.').pop(); const fileName = `${Math.random()}.${fileExt}`; const filePath = `${realUser.id}/${fileName}`;
                const { error: uploadError } = await supabase.storage.from('product-media').upload(filePath, item.file);
                if (!uploadError) { const { data } = supabase.storage.from('product-media').getPublicUrl(filePath); finalImageUrls.push(data.publicUrl); }
            }
        }
        if (finalImageUrls.length === 0) finalImageUrls.push('https://images.pexels.com/photos/356056/pexels-photo-356056.jpeg?auto=compress&cs=tinysrgb&w=300');
        const productData = { seller_id: realUser.id, title: productTitle, description: productDesc, category: productCategory, stock: quantity, images: finalImageUrls, platform_links: { ozon: ozonLink, wb: wbLink }, requirements: { photo: reqPhoto, video: reqVideo }, status: 'Active' };
        if (editingId) {
        ...
        resetForm(); setCurrentView('PRODUCTS');
    } catch (error: any) { alert('Error: ' + error.message); } finally { setIsPublishing(false); }
  };

  const handleDeleteProduct = async (id: string) => {
      if(!confirm("Are you sure?")) return;
      const { error } = await supabase.from('products').delete().eq('id', id);
      if(!error) setRealProducts(prev => prev.filter(p => p.id !== id)); else deleteProduct(id);
  }
  ...
  const resizeImage = (file: File): Promise<Blob> => {
      return new Promise((resolve, reject) => {
          ...
          img.onload = () => {
              const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d'); const size = 400;
              canvas.width = size; canvas.height = size;
              if (!ctx) { reject(new Error("Canvas context failed")); return; }
              const minSize = Math.min(img.width, img.height); const startX = (img.width - minSize) / 2; const startY = (img.height - minSize) / 2;
              ctx.drawImage(img, startX, startY, minSize, minSize, 0, 0, size, size);
              canvas.toBlob((blob) => { if (blob) resolve(blob); else reject(new Error("Canvas conversion failed")); }, 'image/jpeg', 0.9);
          };
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          setAvatarFile(resizedFile); setAvatarUrl(URL.createObjectURL(resizedFile));
          const fileName = `avatars/${realUser.id}_${Date.now()}.jpg`;
          const { error: uploadError } = await supabase.storage.from('product-media').upload(fileName, resizedFile);
      ...
      } catch (error: any) { alert("Error: " + error.message); } finally { setIsUploading(false); e.target.value = ''; }
  };

  const handleSaveSettings = async () => { 
      try {
          const { error } = await supabase.auth.updateUser({ data: { shop_name: shopName } });
          if (error) throw error;
  ...
  const handleUpdatePassword = async () => {
      ...
      try {
          ...
          if (error) throw error;
          alert("Password updated successfully!"); setNewPassword(''); setConfirmPassword('');
      } catch (error: any) { alert("Error: " + error.message); } finally { setIsSecurityLoading(false); }
  };

  const handleUpdateEmail = async () => {
      if (!newEmail.includes('@')) { alert("Invalid email address"); return; }
      setIsSecurityLoading(true);
      try {
          const { error } = await supabase.auth.updateUser({ email: newEmail });
          if (error) throw error;
          alert("Confirmation links sent to both emails."); setNewEmail('');
      } catch (error: any) { alert("Error: " + error.message); } finally { setIsSecurityLoading(false); }
  };
  ...
  const handleDeleteAccount = async () => {
      ...
      try {
          ...
          if (error) throw error;
          alert("Account deleted. Goodbye!"); onLogout();
      } catch (error: any) { alert("Could not delete account: " + error.message); } finally { setIsSecurityLoading(false); }
  };

  const handleProcessPayment = () => { setIsProcessingPayment(true); setTimeout(() => { upgradeSubscription(selectedPlan); setIsProcessingPayment(false); setShowPaymentModal(false); alert('Payment Successful!'); }, 2000); };
  
  const handleKpiClick = (type: 'REVIEWS' | 'PRODUCTS' | 'ORDERS' | 'ANALYTICS') => {
      setOverviewTab(type);
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 flex flex-col md:flex-row relative">
      ...
      {selectedReviewDetail && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                  <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50"><h3 className="font-bold text-gray-800">Review Details</h3><button onClick={() => setSelectedReviewDetail(null)} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><X size={20} className="text-gray-500"/></button></div>
                  <div className="p-6 overflow-y-auto">
                      <div className="flex items-center gap-3 mb-6"><img src={selectedReviewDetail.productImage} className="w-12 h-12 rounded-lg object-cover border border-gray-200" alt="Product"/><div><div className="text-sm text-gray-500">Product</div><div className="font-bold text-gray-800">{selectedReviewDetail.productTitle}</div></div></div>
                      <div className="mb-6"><div className="flex text-yellow-500 mb-2">{[...Array(5)].map((_, i) => (<Star key={i} size={18} fill={i < selectedReviewDetail.rating ? "currentColor" : "none"} />))}</div><p className="text-gray-700 bg-gray-50 p-3 rounded-lg border border-gray-100 italic">"{selectedReviewDetail.content}"</p></div>
                      <div className="mb-6"><label className="block text-xs font-bold text-gray-500 uppercase mb-2">Proof Screenshot</label>{selectedReviewDetail.proofUrl ? (<div className="rounded-xl overflow-hidden border border-gray-200 bg-gray-100"><img src={selectedReviewDetail.proofUrl} alt="Proof" className="w-full h-auto object-contain max-h-64" /></div>) : (<div className="text-sm text-gray-400 italic">No screenshot provided.</div>)}</div>
                      {selectedReviewDetail.externalLink ? (<a href={selectedReviewDetail.externalLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 transition-colors shadow-lg"><ExternalLink size={18} /> View on Ozon/WB</a>) : (<button disabled className="w-full py-3 bg-gray-100 text-gray-400 font-bold rounded-xl cursor-not-allowed">No Link Provided</button>)}
                  </div>
              </div>
          </div>
      )}
      ...
      {showPaymentModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
              <div className="bg-white rounded-2xl w-full max-w-3xl overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
                  <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-purple-50"><h3 className="text-xl font-bold text-gray-800 flex items-center gap-2"><Award className="text-purple-600" />{t.seller.upgradeToPro}</h3><button onClick={() => setShowPaymentModal(false)} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><X size={20} className="text-gray-500" /></button></div>
                  <div className="p-8 flex-1 overflow-y-auto">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                          <div onClick={() => setSelectedPlan('MONTHLY')} className={`border-2 rounded-xl p-6 cursor-pointer transition-all relative ${selectedPlan === 'MONTHLY' ? 'border-purple-600 bg-purple-50' : 'border-gray-200 hover:border-purple-300'}`}>
                              <h4 className="font-bold text-gray-700 text-lg mb-2">{t.seller.monthlyPlan}</h4>
                              <div className="flex items-baseline gap-1"><span className="text-3xl font-bold text-gray-900">$49.99</span><span className="text-gray-500">/mo</span></div>
                              <div className={`w-5 h-5 rounded-full border-2 absolute top-6 right-6 flex items-center justify-center ${selectedPlan === 'MONTHLY' ? 'border-purple-600' : 'border-gray-300'}`}>{selectedPlan === 'MONTHLY' && <div className="w-2.5 h-2.5 bg-purple-600 rounded-full" />}</div>
                          </div>
                          <div onClick={() => setSelectedPlan('YEARLY')} className={`border-2 rounded-xl p-6 cursor-pointer transition-all relative ${selectedPlan === 'YEARLY' ? 'border-purple-600 bg-purple-50' : 'border-gray-200 hover:border-purple-300'}`}>
                              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-sm">{t.seller.bestValue}</div>
                              <h4 className="font-bold text-gray-700 text-lg mb-2">{t.seller.yearlyPlan}</h4>
                              <div className="flex items-baseline gap-1"><span className="text-3xl font-bold text-gray-900">$389.00</span><span className="text-gray-500">/yr</span></div>
                              <div className="text-xs text-green-600 font-bold mt-1">Save ~35%</div>
                              <div className={`w-5 h-5 rounded-full border-2 absolute top-6 right-6 flex items-center justify-center ${selectedPlan === 'YEARLY' ? 'border-purple-600' : 'border-gray-300'}`}>{selectedPlan === 'YEARLY' && <div className="w-2.5 h-2.5 bg-purple-600 rounded-full" />}</div>
                          </div>
                      </div>
                      <h4 className="font-bold text-gray-800 mb-4">{t.seller.selectPayment}</h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                          <button onClick={() => setPaymentMethod('ALIPAY')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${paymentMethod === 'ALIPAY' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}><div className="w-12 h-12 bg-[#1677FF] rounded-xl flex items-center justify-center shadow-sm"><AlipayIcon className="text-white" size={32} /></div><span className="font-bold text-sm text-gray-700">{t.seller.paymentMethods.alipay}</span></button>
                          <button onClick={() => setPaymentMethod('WECHAT')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${paymentMethod === 'WECHAT' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:bg-gray-50'}`}><div className="w-12 h-12 bg-[#09B13C] rounded-xl flex items-center justify-center shadow-sm"><WeChatIcon className="text-white" size={36} /></div><span className="font-bold text-sm text-gray-700">{t.seller.paymentMethods.wechat}</span></button>
                          <button onClick={() => setPaymentMethod('CARD')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${paymentMethod === 'CARD' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:bg-gray-50'}`}><div className="w-12 h-12 bg-gray-900 rounded-xl flex items-center justify-center shadow-sm text-white"><CreditCard size={28} /></div><span className="font-bold text-sm text-gray-700">{t.seller.paymentMethods.card}</span></button>
                      </div>
      ...
      <div className="w-full md:w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
         <div className="p-6 border-b border-gray-100 flex flex-col gap-1"><h2 className="font-bold text-xl text-purple-700">{t.seller.dashboard}</h2><div className="flex items-center gap-3 mt-4"><div className="w-10 h-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center font-bold overflow-hidden border-2 border-white shadow-sm">{avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover"/> : (realUser?.email?.charAt(0).toUpperCase() || 'S')}</div><div className="flex-1 min-w-0"><div className="text-sm font-bold text-gray-900 truncate">{shopName}</div><div className="text-xs text-gray-500 truncate">{realUser?.email}</div></div></div></div>
      ...
      <div className="flex-1 p-4 md:p-8 overflow-y-auto max-h-screen">
        ...
        {currentView === 'OVERVIEW' && (
            <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div onClick={() => handleKpiClick('ANALYTICS')} className={`bg-white p-6 rounded-xl shadow-sm border cursor-pointer hover:shadow-md transition-all ${overviewTab === 'ANALYTICS' ? 'border-purple-500 ring-2 ring-purple-100' : 'border-gray-100'}`}>
                        <p className={`text-sm font-medium mb-2 flex items-center gap-2 ${overviewTab === 'ANALYTICS' ? 'text-purple-600' : 'text-gray-500'}`}><BarChart3 size={16}/> Total Traffic</p>
                        <div className="flex items-baseline gap-2">
                            <h3 className="text-3xl font-bold text-gray-900">{totalViews}</h3>
                            <span className="text-xs text-gray-400">Views</span>
                        </div>
                        <div className="mt-1 text-xs font-bold text-blue-500 flex items-center gap-1"><MousePointerClick size={12}/> {totalClicks} Clicks</div>
                    </div>
                </div>
                ...
                <div ref={overviewContentRef} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden min-h-[400px]">
                    <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        <h3 className="font-bold text-lg text-gray-800">
                            {overviewTab === 'REVIEWS' && t.seller.recentReviews}
                            {overviewTab === 'PRODUCTS' && t.seller.products}
                            {overviewTab === 'ORDERS' && t.seller.orders}
                            {overviewTab === 'ANALYTICS' && 'Traffic Analytics'}
                        </h3>
                    </div>
                    ...
                    {overviewTab === 'ANALYTICS' && (
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-gray-50 text-gray-500 text-sm uppercase"><tr><th className="px-6 py-4 font-medium">Product</th><th className="px-6 py-4 font-medium">Views</th><th className="px-6 py-4 font-medium">Clicks (Ozon/WB)</th><th className="px-6 py-4 font-medium">CTR</th></tr></thead>
                            <tbody className="divide-y divide-gray-100">{displayProducts.map(p => { 
                                const v = p.views_count || 0; const c = p.clicks_count || 0; const ctr = v > 0 ? ((c / v) * 100).toFixed(1) : '0.0';
                                return (<tr key={p.id} className="hover:bg-gray-50 transition-colors"><td className="px-6 py-4 font-medium text-gray-800 flex items-center gap-3"><img src={p.images?.[0]} className="w-8 h-8 rounded object-cover bg-gray-100"/>{p.title}</td><td className="px-6 py-4 text-gray-600">{v}</td><td className="px-6 py-4 text-gray-600">{c}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs font-bold ${parseFloat(ctr) > 5 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{ctr}%</span></td></tr>)
                            })}</tbody>
                        </table>
                    )}
                </div>
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-4">
                             {mediaItems.map((item, idx) => (
                                ...
                                >
                                    ...
                                    <div className="absolute top-1 left-1 p-1 bg-black/30 rounded text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"><GripHorizontal size={14} /></div>
                                    ...
                                    <div className={`absolute bottom-0 left-0 right-0 text-[10px] px-1 py-0.5 text-center text-white ${idx === 0 ? 'bg-purple-600' : (item.type === 'EXISTING' ? 'bg-black/50' : 'bg-green-500')}`}>{idx === 0 ? 'Main Image' : (item.type === 'EXISTING' ? 'Saved' : 'New')}</div>
                                </div>
                             ))}
                        </div>
                    </div>
        ...
        {currentView === 'ANALYTICS' && (
            <div className="space-y-8">
                <div className="flex justify-between items-center"><h1 className="text-3xl font-bold text-gray-800">{t.seller.analytics}</h1><div className="bg-white px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 shadow-sm">Real-time Data</div></div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <h3 className="font-bold text-lg text-gray-800 mb-6">Product Performance</h3>
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50 text-gray-500 text-sm uppercase"><tr><th className="px-6 py-4 font-medium">Product</th><th className="px-6 py-4 font-medium">Views</th><th className="px-6 py-4 font-medium">Clicks</th><th className="px-6 py-4 font-medium">CTR</th></tr></thead>
                        <tbody className="divide-y divide-gray-100">{displayProducts.map(p => { 
                            const v = p.views_count || 0; const c = p.clicks_count || 0; const ctr = v > 0 ? ((c / v) * 100).toFixed(1) : '0.0';
                            return (<tr key={p.id} className="hover:bg-gray-50 transition-colors"><td className="px-6 py-4 font-medium text-gray-800 flex items-center gap-3"><img src={p.images?.[0]} className="w-8 h-8 rounded object-cover bg-gray-100"/>{p.title}</td><td className="px-6 py-4 text-gray-600">{v}</td><td className="px-6 py-4 text-gray-600">{c}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs font-bold ${parseFloat(ctr) > 5 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{ctr}%</span></td></tr>)
                        })}</tbody>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
                    <div className="relative z-10">
                         <div className="flex items-center gap-2 mb-2"><span className={`px-3 py-1 rounded-full text-xs font-bold ${sellerSubscription.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : sellerSubscription.status === 'TRIAL' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}>{sellerSubscription.status}</span><span className="text-sm text-gray-500 font-medium">{sellerSubscription.plan === 'MONTHLY' ? t.seller.monthlyPlan : sellerSubscription.plan === 'YEARLY' ? t.seller.yearlyPlan : 'Free Trial'}</span></div>
                          <h2 className="text-3xl font-bold text-gray-900 mb-6">{isExpired ? t.seller.trialExpired : `${daysRemaining} ${t.seller.daysRemaining}`}</h2>
                          <button onClick={() => setShowPaymentModal(true)} className="px-6 py-3 bg-purple-600 text-white font-bold rounded-xl shadow hover:bg-purple-700 transition-colors">{isExpired ? t.seller.upgradeToPro : 'Extend Subscription'}</button>
                    </div>
                </div>
            </div>
        )}
        ...
        {currentView === 'SETTINGS' && (
            <div className="max-w-4xl mx-auto space-y-8">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-gray-800">{t.seller.settings}</h1>
                    <button onClick={onLogout} className="px-4 py-2 bg-gray-100 text-gray-600 font-bold rounded-lg hover:bg-red-50 hover:text-red-600 flex items-center gap-2 transition-colors"><LogOut size={18}/>{t.seller.logout}</button>
                </div>

                <div className="flex gap-4 border-b border-gray-200 mb-6">
                    <button onClick={() => setActiveSettingsTab('PROFILE')} className={`pb-3 px-2 font-bold text-sm ${activeSettingsTab === 'PROFILE' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700'}`}>Profile & Shop</button>
                ...
                {activeSettingsTab === 'PROFILE' && (
                    <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 animate-in fade-in">
                        ...
                        <div className="flex flex-col items-center mb-8">
                            <div className="relative group cursor-pointer">
                                <div className="w-24 h-24 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center font-bold text-3xl overflow-hidden border-4 border-white shadow-md">
                                    {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" /> : (realUser?.email?.charAt(0).toUpperCase() || 'S')}
                                </div>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            <div className="space-y-2"><label className="text-xs font-bold text-gray-500 uppercase">{t.seller.shopName}</label><input type="text" value={shopName} onChange={(e) => setShopName(e.target.value)} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-purple-200 transition-all" /></div>
                        </div>
                        <div className="mt-8 flex justify-end">
                            <button onClick={handleSaveSettings} className="px-8 py-3 bg-purple-600 text-white font-bold rounded-xl shadow-lg hover:bg-purple-700 flex items-center gap-2"><Save size={20}/>{t.seller.saveChanges}</button>
                        </div>
                    </div>
                )}
                ...
                {activeSettingsTab === 'SECURITY' && (
                    <div className="space-y-6 animate-in fade-in">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <p className="text-sm text-gray-500 mb-4">Current Email: <span className="font-bold text-gray-800">{realUser?.email}</span></p>
